...
class StringField(Field):
    ...
    def validate(self, value):
        ...
        if super().validate(value):
            return (value is None) or (
                isinstance(value, str) and self._validate_length(value)
            )
        else:
            return False
    ...
    def _validate_length(self, value):
        ...
        return (self.maximum_length is None) or (
...
class IntField(Field):
    ...
    def validate(self, value):
        ...
        if super().validate(value):
            return (value is None) or (
                isinstance(value, int) and self._validate_value(value)
            )
        else:
            return False
...
print(f"User JSON={u.to_json()}")

w = User(
    '{"id": 5, "name": "John", "surname": "Smith", "height": 185, "year_born": 1989}'
)

print(f"User ID={w.id}")
print(f"User NAME={w.name}")
